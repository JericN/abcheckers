...
export default function Data() {
  ...
  useEffect(() => {
    if (inViewExploration) {
      explorationAnimation(exploTitleRef.current, { opacity: 1, x: 0 });
    } else {
      explorationAnimation(exploTitleRef.current, { opacity: 0, x: -200 });
    }
  }, [inViewExploration]);

  useEffect(() => {
    if (inViewPreprocessing) {
      preprocessingAnimation(preprocessingRef.current, { opacity: 1, x: 0 });
    } else {
      preprocessingAnimation(preprocessingRef.current, { opacity: 0, x: -200 });
    }
  }, [inViewPreprocessing]);

  useEffect(() => {
    if (inViewStructure) {
      structureAnimation(structureRef.current, { opacity: 1, x: 0 });
    } else {
      structureAnimation(structureRef.current, { opacity: 0, x: -200 });
    }
  }, [inViewStructure]);
