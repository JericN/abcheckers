...
export default function InteractiveTimeSeries({ title, caption, xLabel }) {
    ...
    offset.onChange((latest) => { setInterval(Math.floor(latest)); });
    ...
    return (
        ...
        <div className='w-full max-w-6xl aspect-[2/1] border-4 border-xblack-3 p-5 rounded-3xl flex flex-col items-center'>
            ...
            <motion.div ref={sliderRef} className='border-4 border-[#121212] rounded-2xl w-96 h-11'>
                <motion.div className='w-10 h-10 -mt-[0.9px] bg-slate-900 rounded-2xl' style={{ x }} drag='x' dragConstraints={sliderRef} dragElastic={0} dragMomentum={false}></motion.div>
            </motion.div>
        </div>
