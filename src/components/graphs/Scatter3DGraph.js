...
export default function Scatter3DGraph({ data, title, caption }) {
	...
	return (
		<div className='responsive-widebox max-w-4xl border-4 border-xblack-3 p-5 rounded-3xl flex flex-col items-center'>
			<div className='font-B color-B text-2xl font-bold mb-4'>{title}</div>
			<div className='flex flex-col lg:flex-row gap-2 lg:gap-10'>
				<div
					className={css_button + (dataType == 'raw' ? ' bg-xblack text-xwhite' : ' bg-xwhite text-xblack')}
					onClick={() => {
						setDataType('raw');
					}}
				>
				...
				<div
					className={css_button + (isOutliers ? ' bg-xblack text-xwhite' : ' bg-xwhite text-xblack')}
					onClick={() => {
						setOutliers((isOutliers) => !isOutliers);
					}}
				>
					Remove Outlier
				</div>
