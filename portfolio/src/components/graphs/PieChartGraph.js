...
export default function PieChartGrap({ data, title, caption }) {
	return (
		<div className='responsive-box max-w-xl border-4 border-xblack-3 p-5 rounded-3xl flex flex-col items-center'>
			<div className='font-B color-B text-xl font-bold'>{title}</div>
			<div className='w-3/4 aspect-square '>
				<ResponsiveContainer>
					<PieChart>
						<Pie data={data} cx='50%' cy='50%' labelLine={false} label={renderCustomizedLabel} outerRadius={'90%'} fill='#8884d8' dataKey='value'>
